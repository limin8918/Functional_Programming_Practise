...
object Chain {
  ...
  def foldLeft2[A, B](chain: Chain[A], r: B)(f: (A, B) => B): B = chain match {
    case Nil => r
    case Cons(head, tail) => foldLeft2(tail, f(head, r))(f)
  }
  ...
  def reverseLeft2[A](as: Chain[A]): Chain[A] = {
    foldLeft2(as, Chain[A]())((h, acc) => Cons(h, acc))
  }
}
Chain.reverse(Chain())
Chain.reverseLeft(Chain())
Chain.reverseLeft2(Chain())
Chain.reverse(Chain(1, 2, 3))
Chain.reverseLeft(Chain(1, 2, 3))
Chain.reverseLeft2(Chain(1, 2, 3))